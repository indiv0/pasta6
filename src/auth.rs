pub use db::init_db;
pub use routes::{routes, optional_user};
pub use models::User;
...
mod models {
    ...
    impl User {
        pub fn new(id: i32, created_at: DateTime<Utc>, username: String, password: String, session: Option<String>) -> Self {
            Self {
                id,
...
mod db {
    ...
    pub async fn create_user(db: &DbClient, form: &RegisterForm) -> Result<User, Error> {
        ...
        let query = format!("INSERT INTO {} (username, password) VALUES ($1, $2) RETURNING *", TABLE);
        let row = db
    ...
    pub async fn get_user_by_session_id(db: &DbClient, session: &Session) -> Result<Option<User>, Error> {
...
mod filter {
    ...
    pub async fn get_register(current_user: Option<User>) -> Result<impl warp::Reply, warp::Rejection> {
        Ok(RegisterTemplate{ ctx: TemplateContext::new(current_user) })
    }

    pub async fn post_register(form: RegisterForm, db: DbClient) -> Result<impl warp::Reply, warp::Rejection> {
        ...
        let user = db::create_user(&db, &form).await
        ...
        // encoding?
        let session_cookie = format!("{}={}", SESSION_COOKIE_NAME, serde_json::to_string(&session).unwrap());
        Ok(warp::redirect::redirect(redirect_uri))
           .map(|reply| warp::reply::with_header(reply, warp::http::header::SET_COOKIE, session_cookie))
    }
}
...
mod routes {
    ...
    pub fn optional_user(pool: DbPool) -> impl Filter<Extract = (Option<models::User>,), Error = warp::Rejection> + Clone {
        session::optional_session()
            .and(with_db(pool))
            .and_then(|maybe_session, db| {
                async move {
    ...
    /// GET /register
    fn get_register(pool: DbPool) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
        warp::path("register")
            .and(warp::get())
    ...
    /// POST /register
    fn post_register(pool: DbPool) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
        warp::path("register")
            .and(warp::post())
    ...
    pub fn routes(pool: DbPool) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
        get_register(pool.clone())
            .or(post_register(pool))
    }
}
